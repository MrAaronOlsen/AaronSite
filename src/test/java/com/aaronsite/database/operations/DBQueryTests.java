...
class DBQueryTests extends TestServer {
  ...
  void queryByValueReturnsSingleRecord() throws ABException {
    ...
    try (DBConnection dbConn = new DBConnection()) {
      DbQuery dbQuery = new DbQuery(dbConn, Table.TEST_SIMPLE).setQuery(new DBQueryBuilder().add(NAME, "test"));
  ...
  void queryOnNonExistingValueReturnsNoResults() throws ABException {
    try (DBConnection dbConn = new DBConnection()) {
      DbQuery dbQuery = new DbQuery(dbConn, Table.TEST_SIMPLE).setQuery(new DBQueryBuilder().add("name", "bad"));
