...
class DBInsertStmtBuilderTests extends TestServer {
  ...
  void simpleInsertSQL() throws ABException {
    try (DBConnection conn = new DBConnection()) {
      String expectedSql = "INSERT INTO " + TABLE.apply(TEST_SIMPLE) + " (one, two) VALUES('1', '2') RETURNING *";
  ...
  void unescapedInsertSQL() throws ABException {
    try (DBConnection conn = new DBConnection()) {
      String expectedSql = "INSERT INTO " + TABLE.apply(TEST_SIMPLE) + " (one, two) VALUES('Isn''t', 'Line\n') RETURNING *";
  ...
  void htmlInsertSQL() throws ABException {
    try (DBConnection conn = new DBConnection()) {
      String expectedSql = "INSERT INTO " + TABLE.apply(TEST_SIMPLE) + " (html) VALUES('<div>html</div>') RETURNING *";
